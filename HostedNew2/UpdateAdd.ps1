...
<#	$files = gci -path $($updateObj.WebSitePath) -file -recurse | where -FilterScript { $_.name -eq "harland.key" -or $_.name -eq 'JHKEY.key' -or $_.name -eq 'JHAKEY.key' }
	...
	if ($UpdXml.DocType -ne $null) { $updDocType = $UpdXml.DocType }
	elseif ($InfoObj.DocType -ne $null) { $updDocType = $InfoObj.DocType }
	
	if ($binding -like "*443*")
	{
		if ($updDocType -eq 'CSI') { $newWebCfgPath = "$global:ScriptDirectory\SslWeb.config" }
		elseif ($updDocType -eq 'WKFS') { $newWebCfgPath = "$global:ScriptDirectory\WkSslWeb.config" }
	}
	else
	{
		if ($updDocType -eq 'CSI') { $newWebCfgPath = "$global:ScriptDirectory\NoSslWeb.config" }
		elseif ($updDocType -eq 'WKFS') { $newWebCfgPath = "$global:ScriptDirectory\WkNoSslWeb.config" }
	}
